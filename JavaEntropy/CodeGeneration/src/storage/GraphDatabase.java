...
public class GraphDatabase implements Serializable {
	...
	public void prune() {
		System.out.println("GraphDatabase.prune() " + h1GraphMaps.size());
		...
		for (Integer h1val : h1GraphMaps.keySet()) {
			...
			for (Graph graph : tmp) {
				// GraphDrawingUtils.outputDotFile(graph,
				// "/home/dharani/concordia/thesis/data/" + i + ".dot",
				// globalData);
				i++;
				if (graph.methodIdxList.length > 1) {
					// GraphDrawingUtils.outputDotFile(graph,
					// "/home/dharani/concordia/thesis/data/",
					// globalData);(graph, globalData);
					graphs.add(graph);
				}
			}
			if (graphs.size() > 0) {
...
//			prune(); // uncomment to eliminate graphs which have occurred only in one method.
	...
	public void buildAllChildSimple() {
		...
		//
		//// for (Graph parentGraph:allParentGraphs){
		//// executor.execute(new FastQuery(graph, parentGraph,
		// h1GraphMapsSimple));
		//// }
	...
	public void addGraphWithOtherData(Graph aGraph, int count, String projectName, String methodName) {
		aGraph.count = count;
	...
	public void addGraph(Graph aGraph) {
		...
		if (!h1GraphMaps.containsKey(h1Val)) {
			...
			Graph[] oldList = h1GraphMaps.get(h1Val);
			Graph[] newList = addGraphToList(oldList, aGraph);
	...
	public void doStatistics() {
		...
		Logger.logDebugBis("Number of Graphs: " + numGraphs);

		long numRepGraphs = countRepGraphs();
		Logger.logDebugBis("Number of rep Graphs: " + numRepGraphs);

		// Logger.log("Top Graphs: ");
		// ArrayList<Graph> topGraphList =
		// getTopGraphs(GlobalConfig.topGraphSize);
		...
		// " :");
		// ArrayList<Graph> topSpecGraphList1 =
		// getTopGraphsHaveNodeStartWithStr(GlobalConfig.topGraphSize, 1,
		// GlobalConfig.containStr);
		...
		// " :");
		// ArrayList<Graph> topSpecGraphList2 =
		// getTopGraphsHaveNodeStartWithStr(GlobalConfig.topGraphSize, 2,
		// GlobalConfig.containStr);
		...
		// " :");
		// ArrayList<Graph> topSpecGraphList3 =
		// getTopGraphsHaveNodeStartWithStr(GlobalConfig.topGraphSize, 3,
		// GlobalConfig.containStr);
		...
		// " :");
		// ArrayList<Graph> topSpecGraphList5 =
		// getTopGraphsHaveNodeStartWithStr(GlobalConfig.topGraphSize, 5,
		// GlobalConfig.containStr);
		...
		Logger.logDebugBis(
				"Top  Graphs contains " + GlobalConfig.containStr + " (according to #methods containing them):");
		ArrayList<Graph> topMethodSpecGraphList = getTopMethodGraphsHaveNodeStartWithStr(GlobalConfig.topGraphSize,
				GlobalConfig.maxGraphSize, GlobalConfig.containStr);
		for (Graph graph : topMethodSpecGraphList) {
			Logger.logDebugBis("  #methods:  " + graph.methodIdxList.length + "::" + graph.toString());
		}
	}
