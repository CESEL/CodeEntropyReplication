...
public class GraphQueryEvaluation {
	...
	public TreeMap<Long, Graph> createGraphs(String queryListPath, GraphDatabase graphDB) {
		...
		for (String idx : queryDataList.idxNodesMap.keySet()) {
			...
			Logger.log("idx: " + idx + "///" + count + "////" + dateFormat.format(cal.getTime()));// +
																									// "///nodes:
																									// "
																									// +
																									// nodes);
	...
	public Graph proceedGraphsWithScores(String idx, LinkedHashMap<Graph, Double> graphsWithScores,
		...
		for (Graph graph : rankedGraphs) {
			...
			grs.add(graph);
			count++;
			// String outputPath = graphOutputPath + "/" + count + "_"+
			// graphsWithScores.get(graph) + ".dot";
			// GraphDrawingUtils.outputDotFile(graph, outputPath, globalData);
			//
			// String outputGifPath = graphOutputPath + "/" + count + "_"+
			// graphsWithScores.get(graph) + "_" + graph.getLongestPath() +
			// ".gif";
